...
class UKParliamentService with ChangeNotifier implements ParliamentServiceInterface {
  ...
  final ParliamentCacheManager _cache = ParliamentCacheManager('uk');
  ...
  Future<void> initialize() async {
    ...
    if (hasCache) {
      try {
        ...
        notifyListeners();
         ...
         developer.log('Błąd parsowania cache metadanych UK', name: 'UKParliamentService');
      }
    }
    ...
    try {
      ...
      developer.log('Błąd podczas czyszczenia cache: $e', name: 'UKParliamentService');
      });
...
@override
  ...
  Future<HomeScreenData> getHomeScreenData(BuildContext context, {bool forceRefresh = false}) async {
    ...
    try {
      ...
      developer.log('Błąd sieciowy. Próba fallback offline.', name: 'UKParliamentService');
 ...
 @override
  Future<Map<String, dynamic>> getCivicProjects(
    ...
    try {
      ...
      developer.log('Wywołanie API uk_getCivicProjects z parametrami: $params', name: 'UKParliamentService');
      final resultData = await _apiService.callFunction('uk_getCivicProjects', params: params);
      ...
      developer.log('Błąd w getCivicProjects, próba odczytu z cache: $e', name: 'UKParliamentService');
      // CACHE FALLBACK
      final cachedData = await _cache.getCivicProjects(langCode, limit, lastVisibleId, category, sortBy);
      if (cachedData != null) return cachedData;
      rethrow;
...
@override
  ...
  Future<Legislation?> getLegislationDetails(BuildContext context, String legislationId, {bool forceRefresh = false, String? documentType}) async {
    developer.log('SERVICE: Szczegóły ID: $legislationId, forceRefresh: $forceRefresh, type: $documentType', name: 'UKParliamentService');
    final langCode = Provider.of<LanguageProvider>(context, listen: false).appLanguageCode;
