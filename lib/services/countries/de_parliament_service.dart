...
class DEParliamentService with ChangeNotifier implements ParliamentServiceInterface {
  ...
  final ParliamentCacheManager _cache = ParliamentCacheManager('de');
    ...
    if (hasCache) {
      try {
        ...
        notifyListeners();
         ...
         developer.log('Błąd parsowania cache metadanych DE', name: 'DEParliamentService');
      }
    }
...
@override
  ...
  Future<HomeScreenData> getHomeScreenData(BuildContext context, {bool forceRefresh = false}) async {
    ...
    try {
      ...
      developer.log('Błąd sieciowy. Próba fallback offline.', name: 'DEParliamentService');
 ...
 @override
  Future<Map<String, dynamic>> getCivicProjects(
    ...
    try {
      ...
      developer.log('Wywołanie API us_getCivicProjects z parametrami: $params', name: 'DEParliamentService');
      final resultData = await _apiService.callFunction('de_getCivicProjects', params: params);
      
      await _cache.saveCivicProjects(resultData, langCode, limit, lastVisibleId, category, sortBy);
      return resultData;

    } catch (e) {
      developer.log('Błąd w getCivicProjects, próba odczytu z cache: $e', name: 'DEParliamentService');
      final cachedData = await _cache.getCivicProjects(langCode, limit, lastVisibleId, category, sortBy);
      if (cachedData != null) return cachedData;
      rethrow;
    }
...
Future<Legislation?> getLegislationDetails(BuildContext context, String legislationId, {bool forceRefresh = false, String? documentType}) async {
  developer.log('SERVICE: Szczegóły ID: $legislationId, forceRefresh: $forceRefresh, type: $documentType', name: 'DEParliamentService');
  final langCode = Provider.of<LanguageProvider>(context, listen: false).appLanguageCode;
