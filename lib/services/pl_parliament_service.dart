...
@override
  Future<void> initialize() async {
    ...
    try {
      ...
      developer.log('Używam danych z cache z powodu błędu sieci.', name: 'PLParliamentService');
    } finally {
      _isLoading = false;
      notifyListeners();
      if (!_initializationCompleter.isCompleted) {
...
@override
  ...
  Future<HomeScreenData> getHomeScreenData(BuildContext context, {bool forceRefresh = false}) async {
    ...
    try {
      ...
      developer.log('Błąd sieciowy podczas pobierania danych ekranu głównego: $e', name: 'PLParliamentService');
      final cachedData = await _loadHomeScreenDataFromCache(langCode, termToUse);
      if (cachedData != null) {
        return cachedData;
      } else {
        developer.log('Brak danych ekranu głównego w cache. Rzucam wyjątek.', name: 'PLParliamentService');
...
try {
  ...
  Future<Legislation?> getLegislationDetails(BuildContext context, String legislationId, {bool forceRefresh = false}) async {
    developer.log('SERVICE: Rozpoczynam pobieranie szczegółów ustawy. ID: $legislationId, forceRefresh: $forceRefresh', name: 'PLParliamentService');
    final langCode = Provider.of<LanguageProvider>(context, listen: false).appLanguageCode;
    try {
      if (!forceRefresh) {
...
@override
	...
	Future<void> clearCache() async {
		try {
			...
			final List<String> keysToRemove = allKeys.where((key) => 
				key.startsWith('pl_cached_')
			).toList();
			keysToRemove.add('pl_last_updated');
			developer.log('Znaleziono ${keysToRemove.length} kluczy do usunięcia dla Polski.', name: 'PLParliamentService');
			for (String key in keysToRemove) {
				if (allKeys.contains(key)) {
					await prefs.remove(key);
				}
			}
			developer.log('Cache wyczyszczony dla Polski', name: 'PLParliamentService');
		} catch (e) {
			developer.log('Błąd podczas czyszczenia cache: $e', name: 'PLParliamentService', error: e);
		}
	}
  ...
  String _generateCacheKeyForBillsCursor(
    ...
    return 'pl_cached_bills_cursor_term_${term}_lang_${langCode}_lim_${limit}_cursor_${cursorStr}_stat_${statusStr}_type_${docTypeString}_cat_${categoryStr}_sort_${sortStr}_after_$startDateStr';
  }
    ...
    try {
      ...
      developer.log('Błąd podczas zapisywania cache ustaw (kursor): $e', name: 'PLParliamentService');
    }
  }
...
Future<Map<String, dynamic>?> _getCachedBillsCursor(String langCode, int limit, String? lastVisibleId, String? status, List<String>? documentType, String? category, String? sortBy, String? processStartDateAfter, {required int term}) async {
    try {
      ...
      final lastUpdatedStr = _prefs?.getString('pl_last_updated');
      ...
      developer.log('Błąd podczas odczytu cache (kursor): $e', name: 'PLParliamentService');
      return null;
    }
  }
  ...
  String _generateCacheKeyForMPsCursor(String langCode, int limit, String? lastVisibleId, {required int term, String? club, String? sortBy}) {
    ...
    final cursorStr = lastVisibleId ?? 'firstPage';
    return 'pl_cached_mps_cursor_term_${term}_lang_${langCode}_lim_${limit}_cursor_${cursorStr}_club_${clubKey}_sort_$sortKey';
  }
  ...
  Future<void> _cacheMPsCursor(String langCode, Map<String, dynamic> data, int limit, String? lastVisibleId, {required int term, String? club, String? sortBy}) async {
    try {
      ...
      developer.log('Błąd podczas zapisywania cache posłów (kursor): $e', name: 'PLParliamentService');
    }
  }
  ...
  Future<Map<String, dynamic>?> _getCachedMPsCursor(String langCode, int limit, String? lastVisibleId, {required int term, String? club, String? sortBy}) async {
    try {
      ...
      developer.log('Błąd podczas odczytu cache posłów (kursor): $e', name: 'PLParliamentService');
      return null;
    }
  }

  Future<bool> _loadMetadataFromCache() async {
    final cachedJson = _prefs?.getString('pl_metadata_cache');
    if (cachedJson == null) {
      developer.log('Brak metadanych w cache.', name: 'PLParliamentService');
      return false;
    }
    try {
      ...
      _clubFilters = (data['clubs'] as List<dynamic>?)?.map((e) => e.toString()).toList() ?? [];
      _clubFilters.sort();
      developer.log('Załadowano metadane z cache. Kadencja: $_currentTerm', name: 'PLParliamentService');
      return true;
    } catch (e) {
      developer.log('Błąd podczas ładowania metadanych z cache: $e', name: 'PLParliamentService');
      return false;
    }
  }
  ...
  Future<void> _saveMetadataToCache(Map<String, dynamic> data) async {
    try {
      await _prefs?.setString('pl_metadata_cache', json.encode(data));
      if (data['lastUpdated'] != null) {
        await _prefs?.setString('pl_last_updated', data['lastUpdated'].toString());
      }
      developer.log('Zapisano metadane do cache.', name: 'PLParliamentService');
    } catch (e) {
      developer.log('Błąd podczas zapisywania metadanych do cache: $e', name: 'PLParliamentService');
    }
  }

  Future<void> _saveHomeScreenDataToCache(HomeScreenData data, String lang, int term) async {
    try {
      final key = 'pl_cached_homescreen_term_${term}_lang_$lang';
      await _prefs?.setString(key, json.encode(data.toJson()));
      developer.log('Zapisano dane ekranu głównego do cache (klucz: $key)', name: 'PLParliamentService');
    } catch (e) {
      developer.log('Błąd podczas zapisywania cache ekranu głównego: $e', name: 'PLParliamentService');
    }
  }
  ...
  Future<HomeScreenData?> _loadHomeScreenDataFromCache(String lang, int term) async {
    try {
      ...
      final cachedJson = _prefs?.getString(key);
      if (cachedJson != null) {
        developer.log('Załadowano dane ekranu głównego z cache (klucz: $key)', name: 'PLParliamentService');
        return HomeScreenData.fromJson(json.decode(cachedJson) as Map<String, dynamic>);
      }
    } catch (e) {
      developer.log('Błąd podczas ładowania cache ekranu głównego: $e', name: 'PLParliamentService');
    }
    return null;
  }
  ...
  String key = 'pl_cached_mp_term_${term}_id_${mpId}_lang_$langCode'; 
  if (dataType != null) {
    key += '_data_$dataType';
  }
  if (params != null) {
  ...
  Future<void> _cacheMPDetails(MP mp, {required String langCode, String? dataType, Map<String, dynamic>? params, required int term}) async {
		try {
			...
			developer.log('Zapisano szczegóły posła do cache (klucz: $cacheKey)', name: 'PLParliamentService');
		} catch (e) {
			developer.log('Błąd podczas zapisywania cache szczegółów posła: $e', name: 'PLParliamentService', error: e);
		}
	}

String _generateCacheKeyForLegislationDetails(String legislationId, String langCode) {
  return 'pl_cached_legislation_details_id_${legislationId}_lang_$langCode';
}
...
Future<void> _cacheLegislationDetails(Legislation bill, String langCode) async {
  try {
    ...
    await _prefs?.setString(key, jsonString);
    await _prefs?.setString('${key}_timestamp', DateTime.now().toIso8601String());
    developer.log('Zapisano szczegóły legislacji do cache (klucz: $key)', name: 'PLParliamentService');
  } catch (e) {
    developer.log('Błąd podczas zapisywania cache szczegółów legislacji: $e', name: 'PLParliamentService');
  }
}
...
Future<Legislation?> _getCachedLegislationDetails(String legislationId, String langCode) async {
  try {
    ...
    developer.log('Błąd podczas odczytu cache szczegółów legislacji: $e', name: 'PLParliamentService');
    return null;
  }
}
  ...
  Future<MP?> _getCachedMPDetails(String mpId, {required String langCode, String? dataType, Map<String, dynamic>? params, required int term}) async {
		try {
			...
			final String? lastUpdatedStr = prefs.getString('pl_last_updated');
			if (cachedData == null || timestamp == null || lastUpdatedStr == null) {
				developer.log('Brak szczegółów posła w cache dla klucza: $cacheKey', name: 'PLParliamentService');
				return null;
			}
      ...
      if (cacheTime == null || lastUpdated == null) {
        return null;
			...
			if (cacheTime.isAfter(lastUpdated)) {
				developer.log('Znaleziono aktualne szczegóły posła w cache dla klucza: $cacheKey', name: 'PLParliamentService');
				Map<String, dynamic> mpJson = json.decode(cachedData);
				return MP.fromJson(mpJson);
			}
			developer.log('Szczegóły posła w cache dla klucza: $cacheKey są nieaktualne.', name: 'PLParliamentService');
			return null;
		} catch (e) {
			developer.log('Błąd podczas odczytywania cache szczegółów posła: $e', name: 'PLParliamentService', error: e);
			return null;
		}
	}
