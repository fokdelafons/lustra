...
const HOME_SCREEN_COLLECTION = 'uk_home_screen';
...
const getMetadata = async (req, res) => {
  ...
  try {
    if (req.body.warmup === true) {
      console.log("uk_getMetadata: Otrzymano żądanie rozgrzewające. Zwracam sukces.");
    ...
    console.error(`[uk_getMetadata] Błąd dla zapytania:`, { type: data.type, term: data.term, error: error.message });
...
async function fetchCurrentTerm() {
  try {
    ...
    const doc = await db.collection('uk_metadata').doc('last_update').get();
...
async function fetchTermClubs(term) {
  try {
    ...
    const termClubsDoc = await db.collection('uk_metadata').doc(termClubsDocId).get();
...
async function getHomeScreenDoc(docId, langCode) {
  ...
  try {
    ...
    if (docId !== DOC_ID_TOP_DEPUTIES && originalData) {
      ...
      localizedData.title = originalData[`${langCode}_ai_title`] || originalData[`uk_ai_title`] || originalData.title || 'Brak tytułu';
      localizedData.summary = originalData[`${langCode}_summary`] || originalData[`uk_summary`] || null;
      
      const keyPoints = originalData[`${langCode}_key_points`] || originalData[`uk_key_points`] || [];
...
const getHomeScreenData = async (req, res) => {
  ...
  if (req.body.warmup === true) {
    console.log("uk_getHomeScreenData: Otrzymano żądanie rozgrzewające. Zwracam sukces.");
    return res.status(200).json({ data: { status: 'warmed_up_successfully' } });
  }

  try {
    console.log("Wykonywanie funkcji uk_getHomeScreenData (handler Express)...");
...
const getDetails = async (req, res) => {
    if (req.body.warmup === true) {
        console.log("uk_getDetails: Otrzymano żądanie rozgrzewające.");
        return res.status(200).json({ data: { status: 'warmed_up_successfully' } });
    }
    ...
    try {
        ...
        switch (type) {
            case 'bill':
                const legislationDoc = await db.collection('uk_legislations').doc(id).get();
                ...
                break;
            case 'deputy':
                const deputyDoc = await db.collection('uk_deputies').doc(id).get();
                if (!deputyDoc.exists) {
        ...
        console.error('[uk_getDetails] Błąd:', error);
...
const getDeputies = async (req, res) => {
    if (req.body.warmup === true) {
        console.log("uk_getDeputies: Otrzymano żądanie rozgrzewające.");
        return res.status(200).json({ data: { status: 'warmed_up_successfully' } });
    }
    ...
    try {
        ...
        let query = db.collection('uk_deputies');
        ...
        if (lastVisibleDocId) {
            const lastDoc = await db.collection('uk_deputies').doc(lastVisibleDocId).get();
        ...
        console.error('[uk_getDeputies] Błąd podczas pobierania posłów:', error);
        return res.status(500).json({ error: { code: 'internal', message: 'Wystąpił wewnętrzny błąd serwera.' } });
    }
};
...
const getDeputyDetails = async (req, res) => {
    if (req.body.warmup === true) {
        console.log("uk_getDeputyDetails: Otrzymano żądanie rozgrzewające.");
        return res.status(200).json({ data: { status: 'warmed_up_successfully' } });
    }
    ...
    try {
        ...
        const fetchPromise = (async () => {
            ...
            if (dataType === 'votings') {
                ...
                let interpellationsQuery = db.collection('uk_interpellations')
                    .where('from', 'array-contains', numericDeputyId)
                    .where('term', '==', parseInt(requestedTerm))
                    .orderBy('sentDate', 'desc');
                if (startAfterDocId) {
                    const lastVisibleInterpDoc = await db.collection('uk_interpellations').doc(startAfterDocId).get();
        ...
        console.error('[uk_getDeputyDetails] Błąd:', error);
...
const getLegislations = async (req, res) => {
	if (req.body.warmup === true) {
		console.log("uk_getLegislations: Otrzymano żądanie rozgrzewające.");
		return res.status(200).json({ data: { status: 'warmed_up_successfully' } });
	}
	...
	try {
		...
		const ACCEPTED_STATUS_VARIANTS = ['Act of Parliament', 'Royal Assent'];
		const REJECTED_STATUS_VARIANTS = ["Withdrawn"];
		const PROCESS_STATUS_VARIANTS = ["1st reading", "2nd reading", "3rd reading", "Committee stage", "Commons Examiners", "Lords Special Public Bill Committee", "Money resolution", 'Consideration of Lords amendments', 'Consideration of Lords message', "Order of Commitment discharged", "Programme motion", "Report stage", "Second reading committee", "Ways and Means resolution"];
		...
		let query = db.collection('uk_legislations').where('term', '==', parseInt(requestedTerm, 10));
		...
		if (lastVisibleDocId) {
			const lastDoc = await db.collection('uk_legislations').doc(lastVisibleDocId).get();
			query = query.startAfter(lastDoc);
		}
		...
		const legislations = snapshot.docs.map(doc => {
			return { id: doc.id, ...localizeDocument(doc.data(), lang, defaultLang) };
		});
		...
		console.error('[uk_getLegislations] Error fetching legislations:', error);
...
const citizenVote = async (req, res) => {
    if (req.body.warmup === true) {
        console.log("uk_citizenVote: Otrzymano żądanie rozgrzewające.");
        return res.status(200).json({ data: { status: 'warmed_up_successfully' } });
    }
    ...
    try {
        ...
        if (userData.primaryParliamentId !== 'Parliament of the UK') {
        ...
        let collectionName;
        if (targetType === 'legislation') collectionName = 'uk_legislations';
        else if (targetType === 'deputy') collectionName = 'uk_deputies';
        else {
