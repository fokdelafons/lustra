...
const HOME_SCREEN_COLLECTION = 'fr_home_screen';
...
const getMetadata = async (req, res) => {
  ...
  try {
    if (req.body.warmup === true) {
      console.log("fr_getMetadata: Otrzymano żądanie rozgrzewające. Zwracam sukces.");
    ...
    console.error(`[fr_getMetadata] Błąd dla zapytania:`, { type: data.type, term: data.term, error: error.message });
...
async function fetchCurrentTerm() {
  try {
    ...
    const doc = await db.collection('fr_metadata').doc('last_update').get();
...
async function fetchTermClubs(term) {
  try {
    ...
    const termClubsDoc = await db.collection('fr_metadata').doc(termClubsDocId).get();
...
async function getHomeScreenDoc(docId, langCode) {
  ...
  try {
    ...
    if (docId !== DOC_ID_TOP_DEPUTIES && originalData) {
      ...
      localizedData.title = originalData[`${langCode}_ai_title`] || originalData[`fr_ai_title`] || originalData.title || 'Brak tytułu';
      localizedData.summary = originalData[`${langCode}_summary`] || originalData[`fr_summary`] || null;
      
      const keyPoints = originalData[`${langCode}_key_points`] || originalData[`fr_key_points`] || [];
...
const getHomeScreenData = async (req, res) => {
  ...
  if (req.body.warmup === true) {
    console.log("fr_getHomeScreenData: Otrzymano żądanie rozgrzewające. Zwracam sukces.");
    return res.status(200).json({ data: { status: 'warmed_up_successfully' } });
  }

  try {
    console.log("Wykonywanie funkcji fr_getHomeScreenData (handler Express)...");
...
const getDetails = async (req, res) => {
    if (req.body.warmup === true) {
        console.log("fr_getDetails: Otrzymano żądanie rozgrzewające.");
        return res.status(200).json({ data: { status: 'warmed_up_successfully' } });
    }
    ...
    try {
        ...
        switch (type) {
            case 'bill':
                const legislationDoc = await db.collection('fr_legislations').doc(id).get();
                ...
                break;
            case 'deputy':
                const deputyDoc = await db.collection('fr_deputies').doc(id).get();
                if (!deputyDoc.exists) {
        ...
        console.error('[fr_getDetails] Błąd:', error);
...
const getDeputies = async (req, res) => {
    if (req.body.warmup === true) {
        console.log("fr_getDeputies: Otrzymano żądanie rozgrzewające.");
        return res.status(200).json({ data: { status: 'warmed_up_successfully' } });
    }
    ...
    try {
        ...
        let query = db.collection('fr_deputies');
        ...
        if (lastVisibleDocId) {
            const lastDoc = await db.collection('fr_deputies').doc(lastVisibleDocId).get();
        ...
        console.error('[fr_getDeputies] Błąd podczas pobierania posłów:', error);
        return res.status(500).json({ error: { code: 'internal', message: 'Wystąpił wewnętrzny błąd serwera.' } });
    }
};
...
const getDeputyDetails = async (req, res) => {
    if (req.body.warmup === true) {
        console.log("fr_getDeputyDetails: Otrzymano żądanie rozgrzewające.");
        return res.status(200).json({ data: { status: 'warmed_up_successfully' } });
    }
    ...
    try {
        ...
        const fetchPromise = (async () => {
            ...
            if (dataType === 'votings') {
                ...
                let interpellationsQuery = db.collection('fr_interpellations')
                    .where('from', 'array-contains', numericDeputyId)
                    .where('term', '==', parseInt(requestedTerm))
                    .orderBy('sentDate', 'desc');
                if (startAfterDocId) {
                    const lastVisibleInterpDoc = await db.collection('fr_interpellations').doc(startAfterDocId).get();
        ...
        console.error('[fr_getDeputyDetails] Błąd:', error);
...
const getLegislations = async (req, res) => {
	if (req.body.warmup === true) {
		console.log("fr_getLegislations: Otrzymano żądanie rozgrzewające.");
		return res.status(200).json({ data: { status: 'warmed_up_successfully' } });
	}
	...
	try {
		...
		const ACCEPTED_STATUS_VARIANTS = ["LOI_PUBLIEE", "ORDONNANCE_PUBLIEE", "RÉSOLUTION PUBLIEE", "DECRET PUBLIEE", "DÉCITION"];
		const REJECTED_STATUS_VARIANTS = ["REJETÉ", "PRESCRIT"];
		const PROCESS_STATUS_VARIANTS = ["PROPOSITION_LOI", "PROJET_LOI"];
		...
		let query = db.collection('fr_legislations').where('term', '==', parseInt(requestedTerm, 10));
		...
		if (lastVisibleDocId) {
			const lastDoc = await db.collection('fr_legislations').doc(lastVisibleDocId).get();
			query = query.startAfter(lastDoc);
		}
		...
		console.error('[fr_getLegislations] Error fetching legislations:', error);
...
const citizenVote = async (req, res) => {
    if (req.body.warmup === true) {
        console.log("fr_citizenVote: Otrzymano żądanie rozgrzewające.");
        return res.status(200).json({ data: { status: 'warmed_up_successfully' } });
    }
    ...
    try {
        ...
        if (userData.primaryParliamentId !== 'Assemblée nationale') {
        ...
        let collectionName;
        if (targetType === 'legislation') collectionName = 'fr_legislations';
        else if (targetType === 'deputy') collectionName = 'fr_deputies';
        else {
