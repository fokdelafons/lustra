...
const HOME_SCREEN_COLLECTION = 'de_home_screen';
...
const getMetadata = async (req, res) => {
  ...
  try {
    if (req.body.warmup === true) {
      console.log("de_getMetadata: Otrzymano żądanie rozgrzewające. Zwracam sukces.");
    ...
    console.error(`[de_getMetadata] Błąd dla zapytania:`, { type: data.type, term: data.term, error: error.message });
...
async function fetchCurrentTerm() {
  try {
    ...
    const doc = await db.collection('de_metadata').doc('last_update').get();
...
async function fetchTermClubs(term) {
  try {
    ...
    const termClubsDoc = await db.collection('de_metadata').doc(termClubsDocId).get();
...
async function getHomeScreenDoc(docId, langCode) {
  ...
  try {
    ...
    if (docId !== DOC_ID_TOP_DEPUTIES && originalData) {
      ...
      localizedData.title = originalData[`${langCode}_ai_title`] || originalData[`de_ai_title`] || originalData.title || 'Brak tytułu';
      localizedData.summary = originalData[`${langCode}_summary`] || originalData[`de_summary`] || null;
      
      const keyPoints = originalData[`${langCode}_key_points`] || originalData[`de_key_points`] || [];
...
const getHomeScreenData = async (req, res) => {
  ...
  if (req.body.warmup === true) {
    console.log("de_getHomeScreenData: Otrzymano żądanie rozgrzewające. Zwracam sukces.");
    return res.status(200).json({ data: { status: 'warmed_up_successfully' } });
  }

  try {
    console.log("Wykonywanie funkcji de_getHomeScreenData (handler Express)...");
...
const getDetails = async (req, res) => {
    if (req.body.warmup === true) {
        console.log("de_getDetails: Otrzymano żądanie rozgrzewające.");
        return res.status(200).json({ data: { status: 'warmed_up_successfully' } });
    }
    ...
    try {
        ...
        switch (type) {
            case 'bill':
                const legislationDoc = await db.collection('de_legislations').doc(id).get();
                ...
                break;
            case 'deputy':
                const deputyDoc = await db.collection('de_deputies').doc(id).get();
                if (!deputyDoc.exists) {
        ...
        console.error('[de_getDetails] Błąd:', error);
...
const getDeputies = async (req, res) => {
    if (req.body.warmup === true) {
        console.log("de_getDeputies: Otrzymano żądanie rozgrzewające.");
        return res.status(200).json({ data: { status: 'warmed_up_successfully' } });
    }
    ...
    try {
        ...
        let query = db.collection('de_deputies');
        ...
        if (lastVisibleDocId) {
            const lastDoc = await db.collection('de_deputies').doc(lastVisibleDocId).get();
        ...
        console.error('[de_getDeputies] Błąd podczas pobierania posłów:', error);
        return res.status(500).json({ error: { code: 'internal', message: 'Wystąpił wewnętrzny błąd serwera.' } });
    }
};
...
const getDeputyDetails = async (req, res) => {
    if (req.body.warmup === true) {
        console.log("de_getDeputyDetails: Otrzymano żądanie rozgrzewające.");
        return res.status(200).json({ data: { status: 'warmed_up_successfully' } });
    }
    ...
    try {
        ...
        const fetchPromise = (async () => {
            ...
            if (dataType === 'votings') {
                ...
                let interpellationsQuery = db.collection('de_interpellations')
                    .where('from', 'array-contains', numericDeputyId)
                    .where('term', '==', parseInt(requestedTerm))
                    .orderBy('sentDate', 'desc');
                if (startAfterDocId) {
                    const lastVisibleInterpDoc = await db.collection('de_interpellations').doc(startAfterDocId).get();
        ...
        console.error('[de_getDeputyDetails] Błąd:', error);
...
const getLegislations = async (req, res) => {
	if (req.body.warmup === true) {
		console.log("de_getLegislations: Otrzymano żądanie rozgrzewające.");
		return res.status(200).json({ data: { status: 'warmed_up_successfully' } });
	}
	...
	try {
		...
		const ACCEPTED_STATUS_VARIANTS = ["Bundesrat hat zugestimmt", "Verabschiedet", "Verkündet"];
		const REJECTED_STATUS_VARIANTS = ["Abgelehnt", "Bundesrat hat Zustimmung versagt", "Einbringung abgelehnt", "Für erledigt erklärt", "Erledigt durch Ablauf der Wahlperiode", "Für mit dem Grundgesetz unvereinbar erklärt", "Zurückgezogen"];
		const PROCESS_STATUS_VARIANTS = ["Durchgang im Bundesrat abgeschlossen", "Abgeschlossen", "Abgeschlossen - Ergebnis siehe Vorgangsablauf", "Beschlussempfehlung liegt vor", "Dem Bundesrat zugeleitet - Noch nicht beraten", "Den Ausschüssen zugewiesen", "Noch nicht beraten", "Zusammengeführt mit... (siehe Vorgangsablauf)", "Einbringung beschlossen", "Überwiesen"];
		...
		let query = db.collection('de_legislations').where('term', '==', parseInt(requestedTerm, 10));
		...
		if (statusFilter) {
			...
			if (statusArray.length > 0) {
				...
				if (singleStatusLower === 'accepted') query = query.where('status', 'in', ACCEPTED_STATUS_VARIANTS);
				else if (singleStatusLower === 'rejected') query = query.where('status', 'in', REJECTED_STATUS_VARIANTS);
				else if (singleStatusLower === 'process') query = query.where('status', 'in', PROCESS_STATUS_VARIANTS);
		...
		if (documentTypeFilter) {
			const typesArray = documentTypeFilter.split(',').map(type => type.trim()).filter(type => type.length > 0);
		...
		if (lastVisibleDocId) {
			const lastDoc = await db.collection('de_legislations').doc(lastVisibleDocId).get();
			query = query.startAfter(lastDoc);
		}
		...
		console.error('[de_getLegislations] Error fetching legislations:', error);
...
const citizenVote = async (req, res) => {
    if (req.body.warmup === true) {
        console.log("de_citizenVote: Otrzymano żądanie rozgrzewające.");
        return res.status(200).json({ data: { status: 'warmed_up_successfully' } });
    }
    ...
    try {
        ...
        if (userData.primaryParliamentId !== 'Deutscher Bundestag') {
        ...
        let collectionName;
        if (targetType === 'legislation') collectionName = 'de_legislations';
        else if (targetType === 'deputy') collectionName = 'de_deputies';
        else {
